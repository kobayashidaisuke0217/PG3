...
#include <algorithm>
#include <string>
//文字列内に数字があったら取り出して数字だけを返す
int ConvertAndCompare(const char* str) {
	std::string number;
...
int main() {
	...
	std::sort(idList.begin(), idList.end(), [](const char* a, const char* b) {
	
		//文字列内の数字を比較する
		return ConvertAndCompare(a) < ConvertAndCompare(b);
		});
	for (const char* str : idList) {
		// 学籍番号を表示
		printf("%s\n", str);
	}
	return 0;
